...
# 🔐 Load API key from environment variable (safe practice)
genai.configure(api_key=os.getenv("GEMINI_API_KEY"))

# Load Gemini model
model = genai.GenerativeModel("models/gemini-1.5-flash-latest")

# Sentence generation function using all words at once
...
# Load CSV with predicted classes
df = pd.read_csv("prediction.csv")  # Use relative/local path

# Collect predicted words
predicted_words = df["predicted_class"].tolist()

# Generate sentence
sentence = generate_single_sentence(predicted_words)

# Save sentence to TXT
